...
const Home: NextPage = () => {
  ...
  const [isBuyProductLoading, setIsBuyProductLoading] = useState<boolean>(false)
  ...
  const purchasedProducts = useSWR<IPurchasedProduct[], Error>('purchasedproducts', fetchPurchasedProduct);
        ...
        <div className="flex">
          <div className="flex-1">
            {products && balanceAmount && (
              <Products products={products} handleBuyProduct={handleBuyProduct} balance={balanceAmount} />
            )}
          </div>
          <div className="flex-1 ml-5">
            <ul className="list-none">
              {balance && (
                <li className="font-semibold"><Wallet balance={balance} isBalanceLoading={isBalanceLoading} /></li>
              )}
              <li className="font-semibold"><BankPannel moneyPannel={moneyPannel} handleAddMoneyToWallet={handleAddMoneyToWallet} /></li>
              {purchasedProducts && (
                <li className="font-semibold"><PurchasedProducts purchasedProducts={purchasedProducts} /></li>
              )}
            </ul>
          </div>
        </div>
